...
# Simple GAN for tabular data (small, fast, and PC-friendly)
...
def generate_synthetic_transactions(n_samples=1000, noise_dim=8, epochs=300):
    # Define categories and richer descriptions
    ...
    le = LabelEncoder()
    le.fit(categories)
    # Generate synthetic data with unbiased amounts
    np.random.seed(42)
    synth_rows = []
    for _ in range(n_samples):
        cat = np.random.choice(categories)
        desc = np.random.choice(desc_map[cat])
        # Main distribution: log-normal
        amt = np.random.lognormal(mean=3, sigma=1.1)
        # 5% chance: very small (0.01 to 1)
        if np.random.rand() < 0.05:
            amt = np.random.uniform(0.01, 1)
        # 5% chance: very large (100,000 to 1,000,000)
        elif np.random.rand() < 0.05:
            amt = np.random.uniform(1e5, 1e6)
        # 1% chance: negative (refunds, etc)
        if np.random.rand() < 0.01:
            amt = -amt
        day = np.random.randint(1, 32)
        date = pd.to_datetime('2024-01-01') + pd.to_timedelta(day-1, unit='D')
        # Ensure description always maps to its category
